...
class DatasetBase(torch.utils.data.Dataset):
    ...
    def batch_fn(self, data):
        batch_data = pl.concat(data, how='vertical')
        return {column.name: column.to_list() for column in batch_data.get_columns()}
    ...
    def collate_fn(self, data):
        feed_dict = self.data_fn(data=self.batch_fn(data))
...
class TokenDataset(DatasetBase):
    ...
    def data_fn(self, data):
        tokens, labels = data["token"], data["label"]
        feed_dict = self.tokenizer(
            tokens,
            max_length=self.config.max_seq_length,
            padding="max_length",
...
class StreamDatasetBase(torch.utils.data.IterableDataset):
    ...
    def batch_fn(self, data):
        ...
        {batch_data[key].append(line[key]) for line in data for key in line}
        return batch_data
